# Third-Party Notices

## BOSL2 (Embedded Portions)

This project may include embedded or derived portions from BOSL2.

- Upstream: https://github.com/revarbat/BOSL2
- License: BSD 2-Clause License
- Copyright: (c) 2017-2019, Revar Desmera

If BOSL2-derived code is present in source files, retain original attribution and applicable BSD-2-Clause terms.

## Licensing Interaction Note

Repository-level licensing is `MIT`. Included third-party code remains under its own original license terms, which MIT does not override.
